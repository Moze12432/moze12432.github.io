...
def create_ppt_from_content(title, content, filename="presentation"):
    ...
    try:
        ...
        slide.placeholders[1].text = f"Generated by MozeAI\n{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        
        # Content slides
        content_slide_layout = prs.slide_layouts[1]
        
        # Split content into slides (approx 500 chars per slide)
...
def route(query):
    ...
    if any(x in q for x in ["make a ppt", "create a ppt", "generate ppt", "powerpoint", "make a presentation", "create presentation"]):
        return "create_ppt"
    
    if any(x in q for x in ["make a word", "create a word", "generate word", "word document", "make a doc", "create doc"]):
        return "create_word"
...
def run_agent(query):
    ...
    if tool == "compare_files" and st.session_state.file_context and len(st.session_state.uploaded_files) >= 2:
        ...
        with st.spinner("📊 Creating PowerPoint presentation..."):
            ...
            content_prompt = f"Generate detailed content for a presentation about: {topic}. Include key points, bullet points, and main ideas."
            content = reason(content_prompt, "")
            ...
            if ppt_bytes:
                st.download_button(
                    ...
                    data=ppt_bytes,
                    file_name=f"{topic.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.pptx",
                    mime="application/vnd.openxmlformats-officedocument.presentationml.presentation"
                )
        ...
        with st.spinner("📝 Creating Word document..."):
            ...
            content_prompt = f"Generate detailed content about: {topic}. Include comprehensive information, key points, and organized sections."
            content = reason(content_prompt, "")
            ...
            if word_bytes:
                st.download_button(
                    ...
                    data=word_bytes,
                    file_name=f"{topic.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                )
