...
def create_ppt_from_content(title, content, filename="presentation"):
    ...
    try:
        ...
        for para in paragraphs:
            ...
            if len(first_line) < 60 and (first_line.endswith(':') or first_line.isupper() or len(lines) == 1):
                # New slide
                slide = prs.slides.add_slide(content_slide_layout)
                slide.shapes.title.text = first_line[:100]
                ...
                for line in lines[1:]:
                    if line.strip():
                        ...
                        p.text = line.strip()[:150]
                        p.font.size = Pt(18)
                        p.level = 0
            else:
                # Add to existing slide or create new
                if current_text_frame is None:
                    slide = prs.slides.add_slide(content_slide_layout)
                    slide.shapes.title.text = "Information"
                    content_box = slide.placeholders[1]
                    current_text_frame = content_box.text_frame
                    current_text_frame.text = ""
