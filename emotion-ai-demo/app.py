...
def create_ppt_from_content(title, content, filename="presentation"):
    ...
    try:
        ...
        if "SLIDE" in content.upper():
            ...
            for slide_content in slides[1:]:  # Skip first empty
                if slide_content.strip():
                    lines = slide_content.strip().split('\n')
                    slide_title = lines[0].strip() if lines else "Information"
            ...
            for i, chunk in enumerate(chunks[:7]):  # Max 7 content slides
                slide = prs.slides.add_slide(content_slide_layout)
                slide.shapes.title.text = f"Slide {i + 2}"
                content_box = slide.placeholders[1]
                text_frame = content_box.text_frame
                text_frame.text = chunk[:400]
